...
// 异步的进行 conenct  0为成功， -1 && EINPROGRESS 连接中， 其他为error
...
void TcpClient::connect(std::function<void()> done) {
    ...
    if (rt == 0) {
        ...
        if (errno == EINPROGRESS) {  // 表示处于连接中
            // epoll 监听可写事件，然后判断错误码
            m_fd_event->listen(FdEvent::OUT_EVENT,
                               [this, done]() {
                                   //当有可写事件的时候，再去connect一下
                                   int rt = ::connect(m_fd, m_peer_addr->getSockAddr(), m_peer_addr->getSocketLen());
                                   if ((rt < 0 && errno == EISCONN) || (rt == 0)) {  // 表示连接成功
                                       DEBUGLOG("connect [%s] sussess", m_peer_addr->toString().c_str());
                                       initLocalAddr();
                                       m_connection->setState(Connected);
                                   } else {  // 连接失败
...
void TcpClient::addTimerTask(TimerTask::s_ptr timer_event) {
    m_event_loop->addTimerTask(timer_event);
}
