...
#include "src/reactor/fd_event_group.h"
...
TcpConnection::TcpConnection(EventLoop::s_ptr event_loop, int fd, int buffer_size, NetAddr::s_ptr peer_addr, NetAddr::s_ptr local_addr, TcpConnectionType type /*= TcpConnectionByServer*/)
...
void TcpConnection::onRead() {
  ...
  while(!is_read_all) {
    if (m_in_buffer->writeAble() == 0) {
      m_in_buffer->expandBuffer(2 * m_in_buffer->m_buffer.size());
    }
...
void TcpConnection::excute() {
  ...
  if (m_connection_type == TcpConnectionByServer) {
    ...
    for (size_t i = 0;  i < result.size(); ++i) {
      ...
      // message->m_pb_data = "hello. this is seedrpc rpc test data";
