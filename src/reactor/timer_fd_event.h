#ifndef SRC_REACTOR_TIMER_FD_EVENT_H
#define SRC_REACTOR_TIMER_FD_EVENT_H
...
public:
    typedef std::shared_ptr<TimerFdEvent> s_ptr;
    TimerFdEvent() {
    ...
    std::pair<time_t, uint64_t> addTimerTask(TimerTask::s_ptr timeTask) {
        ...
        if (m_pending_timertasks.empty())
            reset_timefd = true;
        else {
            auto it = m_pending_timertasks.begin();
        ...
        m_pending_timertasks.emplace(timeTask->get_expire(), std::move(timeTask));
        lock.unlock();
    }
    // 删除定时任务
    void deleteTimerEvent(TimerTask::s_ptr timeTask) {
        ...
        ScopeMutex<Mutex> lock(m_mutex);
        auto begin = m_pending_timertasks.lower_bound(timeTask->get_expire());
        auto end = m_pending_timertasks.upper_bound(timeTask->get_expire());
        auto it = begin;
    ...
    void onTimer() {  // 当发送了 IO 事件后，eventloop 会执行这个回调函数
        ...
        ScopeMutex<Mutex> lock(m_mutex);
        auto it = m_pending_timertasks.begin();
        for (; it != m_pending_timertasks.end(); ++it) {
        ...
        m_pending_timertasks.erase(m_pending_timertasks.begin(), it);
        lock.unlock();
        // 重复的需要加入
...
private:
    std::multimap<time_t, TimerTask::s_ptr> m_pending_timertasks;
    Mutex m_mutex;
};
